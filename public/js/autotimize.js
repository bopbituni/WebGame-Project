      ...
      if (documentHeight < offset + 1000) {
        if (loading === false && finished === false) {
          $.ajax({
            ...
            success: function success(data, textResponse, jqXHR) {
              count++;
              var post = $(data);
              ...
              if (post.find(".next-link a").length == 0) finished = true;
  ...
  var e = function () {
    ...
    return s.definePosition = function () {
      ...
      for (var s = 0; s < this.instances.length; s += 1) {
        ...
        if (this.computeScrollOffsets(e), t) for (var i in t) {
          e.props[i] = t[i];
        }
      }
...
(function ($) {
  ...
  function runSlideBanner() {
    ...
    if (jQuery('.breadcrumb').length > 0) delta = delta + jQuery('.breadcrumb').height();
  ...
  jQuery(document).ready(function () {
    jQuery(window).resize(setSlideBannerOffset); // stickybits('.sticky');
...
/******/ 	var __webpack_exports__ = __webpack_require__("./resources/js/autotimize.js");
/******/ 	
/******/ })()
;